...
static bfd_boolean nds32_elf_print_private_bfd_data (bfd *, PTR);
...
static bfd_boolean readonly_dynrelocs (struct elf_link_hash_entry *, PTR);
...
static bfd_boolean nds32_elf_ex9_replace_instruction (bfd *, asection *);
static bfd_boolean nds32_elf_ifc_calc (bfd *, asection *);
static bfd_boolean nds32_elf_ifc_replace (void);
...
static int is_relax_omit_fp = 1;
static int is_insn16_relax_omit_fp = 1;
static int bak_insn16_relax_omit_fp;
static int is_insn_insn16_relax_omit_fp;

static int relax_active = 0;

/* Convert ELF-VER in eflags to string for debugging purpose.  */
static const char *nds32_elfver_strtab[] = {
  "ELF-1.2",
  "ELF-1.3",
  "ELF-1.4",
};

/* By default load/store relaxtion is on for static linked executable, shared
   objec and PIE.  For dynamic linked executable, load/store relaxation is off.  */

/* These are target options set by emulation.
   See nds32elef.em for details.  */

#define FP_BASE_NAME "_FP_BASE_"
int relax_omit_fp = 1;			/* --mrelax-omit-fp  */
int relax_reduce_fp_update = 1;		/* --mrelax-reduce-fp-update  */
int elf32nds32_eliminate_gc_relocs = 0;	/* --meliminate-gc-relocs  */
FILE *sym_ld_script = NULL;		/* --mgen-symbol-ld-script=<file>  */
static int check_start_export_sym = 0;
/* Disable if linking a dynamically linked executable.  */
int elf32nds32_load_store_relax = 1;
/* EX9 Relaxation.  */
int target_optimize = 0;		/* Switch optimization.  */
int relax_status = 0;			/* Finished optimization.  */
int relax_round = 0;			/* Going optimization.  */
FILE *ex9_export_file = NULL;		/* --mexport-ex9=<file>  */
FILE *ex9_import_file = NULL;		/* --mimport-ex9=<file>  */
int update_ex9_table = 0;		/* --mupdate-ex9.  */
size_t ex9_relax_size = 0;		/* Save ex9 predicted reducing size.  */
int ex9_limit = 511;
bfd_boolean ex9_loop_aware = FALSE;	/* Ignore ex9 if inside a loop.  */
/* Link-time IFC options.  */
/* Jump IFC.  */
int ifc_loop_aware = 0;			/* Ignore ifc if inside a loop.  */
...
static reloc_howto_type nds32_elf_howto_table[] = {
  ...
  /* alignment hint for relaxable instruction */
  /* this is used with R_NDS32_LABEL as a pair */
  /* relax this instruction from 4 bytes to 2 */
  /* in order to make next label aligned on word */
  /* boundary */
			   ...
			   asection **secp, bfd_vma *valp)
{
  switch (sym->st_shndx)
	...
	break;
      ...
      /* COLE: st_value is the alignemnt constraint.
  ...
  if (!h || (h->type != bfd_link_hash_defined && h->type != bfd_link_hash_defweak))
    {
      ...
      bfd_vma sda_base;
      /* In general, the first is .data and the final one is .bss.  */
      char sec_name[10][10] = { ".data", ".sdata_d", ".sdata_w", ".sdata_h",
	  ".sdata_b", ".sbss_b", ".sbss_h", ".sbss_w", ".sbss_d", ".bss" };
      size_t i = 0;
	...
	return FALSE;
      ...
      if (relax_omit_fp)
	{
	  h2 = bfd_link_hash_lookup (info->hash, "_FP_BASE_",
				     FALSE, FALSE, FALSE);
	  if (!_bfd_generic_link_add_one_symbol
		 (info, output_bfd, "_FP_BASE_", BSF_GLOBAL | BSF_WEAK,
		  first, (bfd_vma) sda_base - 256, (const char *) NULL,
...
#define nds32_elf_hash_table(p) \
  ((struct elf_nds32_link_hash_table *) ((p)->hash))
...
nds32_elf_link_hash_table_create (bfd *abfd)
{
  ...
  ret = (struct elf_nds32_link_hash_table *) bfd_malloc (amt);
  if (ret == NULL)
    return NULL;
...
static bfd_boolean
nds32_elf_output_symbol_hook (struct bfd_link_info *info ATTRIBUTE_UNUSED,
			      const char *name,
  ...
  for (rel = relocs; rel < relend; rel++)
	...
	case R_NDS32_LO12S0_ORI_RELA:
	  if (info->shared
	      ...
	      && (elf32nds32_eliminate_gc_relocs == 0
  ...
  if (sec_t && sec_t->size >= 4)
    {
      ...
      /* COLE: The value could only be 4 or 16.  */
...
/* Write VAL in uleb128 format to P, returning a pointer to the
   following byte.
   COLE: This code is copied from elf-attr.c.  */
			...
			int *pinsn_type)
{
  ...
  switch (op6)
    ...
    case N32_OP6_SUBRI:
      if (mach >= MACH_V3 && N32_IS_RT3 (insn) && N32_IS_RT3 (insn)
...
special_convert_32_to_16 (unsigned long insn, uint16_t *pinsn16,
			  Elf_Internal_Rela *reloc, int estimate)
{
  uint16_t insn16 = 0;

  if ((estimate || (reloc->r_addend & R_NDS32_INSN16_FP7U2_FLAG))
      && (ELF32_R_TYPE (reloc->r_info) == R_NDS32_INSN16))
    {
      if (!is_insn16_relax_omit_fp)
	return 0;
...
#define SPECIAL_32_TO_16 (1 << 1)
  ...
  else if (special_convert_32_to_16 (insn, insn16, reloc, 0))
  ...
  for (; pc_rel < irelend && pc_rel->r_offset == offset; pc_rel++)
	       ...
	       && convert_type == SPECIAL_32_TO_16)
	{
	  ...
	  mem_addr = calculate_memory_address (abfd, pc_rel, isymbuf,
					       symtab_hdr);
	  /* If mem_addr is smaller than sda, the range of lwi37 have to
	     consider that alignment may change the distance between two
	     sections.  */
	  if ((mem_addr > local_sda
	      ? (mem_addr - local_sda) > 0xef : (local_sda - mem_addr) > 0xd0))
	    return FALSE;
	  break;
	}
...
nds32_elf_blank_t *blank_free_list = NULL;
  ...
  for (sect = abfd->sections; sect != NULL; sect = sect->next)
      ...
      for (irel = internal_relocs; irel < irelend; irel++)
	{
	  ...
	  if (ELF32_R_TYPE (irel->r_info) >= R_NDS32_DIFF8
	    ...
	    {
	      unsigned long val, before, between;
...
bfd_signed_vma align_addend = 0;
/* Range of small data.  */
bfd_vma sdata_range[5][2];
bfd_vma sdata_init_range[5] = { 0x2000, 0x4000, 0x8000, 0x10000, 0x40000 };
...
relax_range_measurement (bfd *abfd)
{
  ...
  /* Rufus: I guess we can not determine the section before
			 ...
			 struct bfd_link_info *link_info, bfd_boolean *again)
{
  ...
  *again = FALSE;
  ...
  /* 09.12.11 Workaround.  */
  /* Rufus: We have to adjust align for R_NDS32_LABEL if needed.
  ...
  switch (relax_round)
    ...
    case NDS32_RELAX_JUMP_IFC_ROUND:
      ...
      if (!nds32_elf_ifc_calc (abfd, sec))
	return FALSE;
      return TRUE;
    ...
    case NDS32_RELAX_EX9_REPLACE_ROUND:
      if (!nds32_elf_ex9_replace_instruction (abfd, sec))
	return FALSE;
      return TRUE;
  ...
  if (ELF32_R_TYPE (irel->r_info) == R_NDS32_RELAX_ENTRY)
	...
	optimize = 1;
      ...
      /* COLE: Is this bit obsoleted?  */
      if (irel->r_addend & R_NDS32_RELAX_ENTRY_OPTIMIZE_FOR_SPACE_FLAG)
	optimize_for_space = 1;

      if (irel->r_addend & R_NDS32_RELAX_ENTRY_OPTIMIZE_FOR_SPACE_UNALIGN_FLAG)
	optimize_for_space_no_align = 1;
    }
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
      ...
      else if (ELF32_R_TYPE (irel->r_info) == R_NDS32_LONGCALL3)
	{
	  ...
	  if (foff >= -0x10000 - 4 && foff < 0x10000 - 4)
	  ...
	  else
	    {
	      ...
	      bfd_putb32 (insn, contents + irel->r_offset);
	      /* COLE: This relax is incorrect.  Review, fix and test it.
      ...
      else if (ELF32_R_TYPE (irel->r_info) == R_NDS32_LOADSTORE)
	{
	  ...
	  switch (ELF32_R_TYPE (hi_irelfn->r_info))
		...
		calculate_memory_address (abfd, hi_irelfn, isymbuf,
	      ...
	      if (((ls_range_type & 0x3f) == 0x20) && (access_addr < 0x7f000))
		{
		  eliminate_sethi = 1;
		  break;
		}

	      if (!elf32nds32_load_store_relax)
		continue;
	       ...
	       || ELF32_R_TYPE (irel->r_info) == R_NDS32_LO12S2_RELA)
	{
	  ...
	  else if (elf32nds32_load_store_relax)
	    {
	      range_l = sdata_range[4][0];
	      range_h = sdata_range[4][1];
	      switch (ELF32_R_TYPE (irel->r_info))
      ...
      else if (ELF32_R_TYPE (irel->r_info) == R_NDS32_PTR)
	{
	  ...
	  if (i1_irelfn->r_addend & 1)
	    {
	      /* Pointed target is relaxed and no longer needs this PTR,
  ...
  if (relax_omit_fp)
    {
	 ...
	 apply this optimization.  */
      ...
      for (irel = internal_relocs; irel < irelend && relax_reduce_fp_update;
	   irel++)
	{
	  if (ELF32_R_TYPE (irel->r_info) == R_NDS32_RELAX_REGION_BEGIN
		...
		(*_bfd_error_handler) (_("%B: Nested OMIT_FP in %A."), abfd, sec);
	      ...
	      fp_use = 0;
		   ...
		   && (irel->r_addend & R_NDS32_RELAX_REGION_OMIT_FP_FLAG))
	    {
	      if (fp_use < 3 && fp_use > 0 && begin_rel != NULL)
		begin_rel->r_addend = begin_rel->r_addend | R_NDS32_RELAX_REGION_NOT_OMIT_FP_FLAG;
	      begin_rel = NULL;
	      fp_use = 0;
	    }
	  ...
	  if (ELF32_R_TYPE (irel->r_info) == R_NDS32_SDA15S2_RELA
	    ...
	    {
	      access_addr = calculate_memory_address (abfd, irel, isymbuf, symtab_hdr);
	      if ((access_addr > local_sda ? (access_addr - local_sda) < 0xf0
		   : (local_sda - access_addr) <= 0xf0))
		fp_use++;
	    }
      ...
      /* Set R_NDS32_INSN16_FP7U2_FLAG for each gp-load,
	 if we want to apply this optimization in that region.  */
      ...
      for (irel = internal_relocs; irel < irelend; irel++)
	{
	  if (ELF32_R_TYPE (irel->r_info) == R_NDS32_RELAX_REGION_BEGIN
	      && (irel->r_addend & R_NDS32_RELAX_REGION_OMIT_FP_FLAG))
	    {
	      omit_fp_on = !(irel->r_addend & R_NDS32_RELAX_REGION_NOT_OMIT_FP_FLAG);
	    }
	  else if (ELF32_R_TYPE (irel->r_info) == R_NDS32_RELAX_REGION_END
		   && (irel->r_addend & R_NDS32_RELAX_REGION_OMIT_FP_FLAG))
	    {
	  ...
	  if (ELF32_R_TYPE (irel->r_info) == R_NDS32_SDA15S2_RELA
		...
		find_relocs_at_address (irel, internal_relocs, irelend,
					R_NDS32_INSN16);
	      if (tmp_rel != irelend)
		tmp_rel->r_addend |= R_NDS32_INSN16_FP7U2_FLAG;
	    }
	}
    }
  ...
  if (*again == FALSE)
    {
      /* This code block remove unused `la $fp, _FP_BASE_' instruction.  */
      Elf_Internal_Rela *begin_rel = NULL;
      Elf_Internal_Rela *tmp_rel;
      int eliminate_fp_update = 1, delete_fp_update = 0;

      for (irel = internal_relocs;
	   irel < irelend && relax_omit_fp && relax_reduce_fp_update;
	   irel++)
	{
	  if (ELF32_R_TYPE (irel->r_info) == R_NDS32_RELAX_REGION_BEGIN
	      && (irel->r_addend & R_NDS32_RELAX_REGION_OMIT_FP_FLAG)
	    ...
	    begin_rel = irel;
		   ...
		   && (irel->r_addend & R_NDS32_RELAX_REGION_OMIT_FP_FLAG))
	    {
	      if (eliminate_fp_update && begin_rel != NULL)
		{
		  for (tmp_rel = begin_rel; tmp_rel < irel; tmp_rel++)
		    {
		      if (ELF32_R_TYPE (tmp_rel->r_info) == R_NDS32_SDA19S0_RELA
			  && ELF32_R_SYM (tmp_rel->r_info) >= symtab_hdr->sh_info
			  && strcmp (elf_sym_hashes (abfd)
				     [ELF32_R_SYM (tmp_rel->r_info) -
				     symtab_hdr->sh_info]->root.root.string,
				     FP_BASE_NAME) == 0)
			{
			  /* addi.gp  $fp, -256 */
			  insn = bfd_getb32 (contents + tmp_rel->r_offset);
			  if (insn == INSN_ADDIGP_TO_FP)
			    delete_fp_update = 1;
			}
		      else if (ELF32_R_TYPE (tmp_rel->r_info) == R_NDS32_20_RELA
			       && ELF32_R_SYM (tmp_rel->r_info) >= symtab_hdr->sh_info
			       && strcmp (elf_sym_hashes (abfd)
					  [ELF32_R_SYM (tmp_rel->r_info) -
					  symtab_hdr->sh_info]->root.
					  root.string, FP_BASE_NAME) == 0)
			{
			  /* movi  $fp, FP_BASE */
			  insn = bfd_getb32 (contents + tmp_rel->r_offset);
		      ...
		      if (delete_fp_update)
			{
			  ...
			  if (!insert_nds32_elf_blank_recalc_total
			      (&relax_blank_list, tmp_rel->r_offset, 4))
			    goto error_return;
			  delete_fp_update = 0;
			}
		    }
	      ...
	      begin_rel = NULL;
	      eliminate_fp_update = 1;
	    }

	  if (begin_rel != NULL)
	    {
	      if (ELF32_R_TYPE (irel->r_info) == R_NDS32_SDA_FP7U2_RELA
		  || ELF32_R_TYPE (irel->r_info) == R_NDS32_17IFC_PCREL_RELA
		  || ELF32_R_TYPE (irel->r_info) == R_NDS32_10IFCU_PCREL_RELA)
		eliminate_fp_update = 0;
	    }
	}
    }
  ...
  if (*again == FALSE)
      ...
      for (tmp_rel = internal_relocs; tmp_rel < irelend; tmp_rel++)
	{
	 ...
	 all reloc entries has been sorted by r_offset.  */
      for (irel = internal_relocs; irel < irelend; irel++)
	{
	  if (relax_reduce_fp_update)
	    {
	      if (ELF32_R_TYPE (irel->r_info) == R_NDS32_RELAX_REGION_BEGIN
		  && (irel->r_addend & R_NDS32_RELAX_REGION_OMIT_FP_FLAG))
		{
		  if ((irel->r_addend & R_NDS32_RELAX_REGION_NOT_OMIT_FP_FLAG))
		    is_relax_omit_fp = 0;
		}
	      else if (ELF32_R_TYPE (irel->r_info) == R_NDS32_RELAX_REGION_END
		       && (irel->r_addend & R_NDS32_RELAX_REGION_OMIT_FP_FLAG))
	  ...
	  if (ELF32_R_TYPE (irel->r_info) == R_NDS32_INSN16)
	      ...
	      else if (insn_rel)
		{
		  ...
		  bak_insn16_relax_omit_fp = is_insn16_relax_omit_fp;
		  is_insn16_relax_omit_fp = is_insn_insn16_relax_omit_fp;
		  ...
		  is_insn_insn16_relax_omit_fp = 0;
		  is_insn16_relax_omit_fp = bak_insn16_relax_omit_fp;
		  insn_rel->r_info =
		  ...
		  (abfd, sec, irel, internal_relocs, irelend, &insn16,
		   local_sda) || is_16bit_NOP (abfd, sec, irel))
		{
		  insn_rel = irel;
		  is_insn_insn16_relax_omit_fp = is_insn16_relax_omit_fp;
		  is_insn16_relax_omit_fp = is_relax_omit_fp;
		}
    ...
    /* Rufus: It doesn't matter optimize_for_space_no_align anymore.
...
nds32_elf_output_arch_syms (bfd *output_bfd ATTRIBUTE_UNUSED,
			    struct bfd_link_info *info ATTRIBUTE_UNUSED,
			    PTR finfo ATTRIBUTE_UNUSED,
			    bfd_boolean (*func) (PTR, const char *,
						 Elf_Internal_Sym *,
						 asection *,
...
struct elf_nds32_ifc_symbol_entry *ifc_symbol_head = NULL;
			     ...
			     unsigned long insn)
{
  ...
  while (ptr != NULL)
	  ...
	  && ptr->insn == insn)
	{
	  ...
	  struct elf_nds32_ifc_irel_list *ptr_irel = ptr->irel_head;

	  while (ptr_irel->next != NULL)
	    ptr_irel = ptr_irel->next;
	  ptr_irel->next = bfd_malloc (sizeof (struct elf_nds32_ifc_irel_list));
	  ptr_irel = ptr_irel->next;
	  ptr_irel->irel = irel;
	  ptr_irel->keep = 1;
...
static bfd_boolean
nds32_elf_ifc_calc (bfd *abfd, asection *sec)
{
  Elf_Internal_Rela *internal_relocs;
  Elf_Internal_Rela *irelend;
  Elf_Internal_Rela *irel;
...
struct bfd_hash_table ex9_code_table;
struct elf_nds32_insn_times_entry *ex9_insn_head = NULL;
struct elf_nds32_ex9_refix *ex9_refix_head = NULL;
...
static void
nds32_elf_order_insn_times (void)
{
  struct elf_nds32_insn_times_entry* ptr;
  struct elf_nds32_insn_times_entry* temp;
  int number = 1;
  ...
  ptr = ex9_insn_head;
  while (ptr != NULL && number <= ex9_limit)
    {
      ptr->order = number;
      number++;
      if (number > ex9_limit)
...
static void
nds32_elf_build_itable (struct bfd_link_info *link_info)
{
  asection *table_sec;
  struct elf_nds32_insn_times_entry *ptr;
  bfd *it_abfd;
  ...
  for (it_abfd = link_info->input_bfds; it_abfd != NULL;
      ...
      if (table_sec != NULL)
	{
	  ...
	  table_sec->size += 4;
	  elf_elfheader (link_info->output_bfd)->e_flags |= 0x100;
	  number = 1;
	  for (ptr = ex9_insn_head; ptr !=NULL ; ptr = ptr->next)
	    {
	      long val;
...
static int
nds32_elf_ex9_relocation_check (Elf_Internal_Rela **irel,
				Elf_Internal_Rela *irelend,
...
static bfd_boolean
nds32_elf_ex9_replace_instruction (bfd *abfd, asection *sec)
{
  struct elf_nds32_insn_times_entry *ptr = ex9_insn_head;
  bfd_byte *contents = NULL;
  long unsigned int off;
  ...
  while (off < sec->size)
	...
	irel++;
      ...
      data_flag = nds32_elf_ex9_relocation_check (&irel, irelend, relax_blank_list,
						  sec, &off, contents);
      ...
      else
	{
	  ...
	  while (ptr)
	    {
	      it_insn = strtol (ptr->string, NULL, 16);
	      it_insn_with_reg = 0;
	      do_replace = 0;
	      save_irel = 0;
	      ...
	      if (irel != NULL && irel < irelend && irel->r_offset == off)
		{
		  ...
		  if (ptr->irel != NULL)
		    {
		      nds32_elf_get_insn_with_reg (ptr->irel, it_insn, &it_insn_with_reg);
		      if (ELF32_R_TYPE (irel->r_info) == ELF32_R_TYPE (ptr->irel->r_info)
			  && (insn_with_reg == it_insn_with_reg))
			{
			  ...
			  if (ELF32_R_TYPE (irel->r_info) == R_NDS32_25_PCREL_RELA
			      ...
			      if (r_symndx < symtab_hdr->sh_info)
				{
				  ...
				  if (ptr->sec == isec
				      && ptr->irel->r_addend == irel->r_addend
				      && ptr->irel->r_info == irel->r_info)
				    {
				      do_replace = 1;
				      save_irel = 1;
				    }
				  ...
				  if (ptr->m_list)
				    {
				      ...
				      h_list = ptr->m_list->h_list;
				      while (h_list)
					{
					  if (h == h_list->h
					      && ptr->m_list->irel->r_addend == irel->r_addend)
					    {
					      do_replace = 1;
					      save_irel = 1;
					      break;
...
static void
nds32_elf_ex9_total_relax (void)
{
  struct elf_nds32_insn_times_entry *ex9_ent;
  struct elf_nds32_insn_times_entry *temp;

  if (ex9_insn_head == NULL)
    return;

  ex9_ent = ex9_insn_head;
  while (ex9_ent)
    {
      ex9_relax_size = ex9_ent->times * 2 + ex9_relax_size;
      temp = ex9_ent;
      ex9_ent = ex9_ent->next;
      free (temp);
    }
  ex9_insn_head = NULL;
			  ...
			  struct bfd_link_info *link_info)
{
  ...
  if (ptr->m_list != NULL)
  ...
  else if (ptr->sec !=NULL)
    {
      ...
      symtab_hdr = &elf_tdata (ptr->sec->owner)->symtab_hdr;
      isym = (Elf_Internal_Sym *) symtab_hdr->contents;
      isym = isym + ELF32_R_SYM (ptr->irel->r_info);

      sec = bfd_section_from_elf_index (ptr->sec->owner, isym->st_shndx);
      if (sec != NULL)
	*isec_ptr = *sec;
      sym = *isym;
...
void
nds32_elf_ex9_import_table (struct bfd_link_info *link_info ATTRIBUTE_UNUSED)
{
  int count = 0, num = 1;
  bfd_byte *contents;
  unsigned long insn;
  ...
  if (update_ex9_table == 1)
    {
      /* Rufus: it has to consider of sethi need to use multiple page
...
static void
nds32_elf_ex9_export (bfd_byte *contents, int size)
{
...
nds32_elf_ex9_reloc_jmp (struct bfd_link_info *link_info)
{
  ...
  if (update_ex9_table == 0)
      ...
      if (table_sec != NULL)
	{
	  ...
	  /* Put the value of _ITB_BASE_ in first itable entry.
	     And insert a symbol at second entry, so objdump can start
	     disassembling at proper boundary.  */
	  relocation = table_sec->output_section->vma;

	  bfd_put_32 (it_abfd, relocation, contents);
	  bh = bfd_link_hash_lookup (link_info->hash, "ex9_entry_1",
				     FALSE, FALSE, TRUE);
	  _bfd_generic_link_add_one_symbol (link_info,
					    output_bfd,
					    "ex9_entry_1",
					    BSF_GLOBAL | BSF_WEAK, table_sec,
  ...
  while (ptr)
    {
      ...
      insn = strtol (ptr->string, NULL, 16);
      insn_with_reg = 0;
      if (ptr->m_list != NULL || ptr->sec != NULL)
	{
	  if (ptr->m_list)
	    rel_backup = ptr->m_list->rel_backup;
	  else
	    rel_backup = ptr->rel_backup;
	  ...
	  if (ELF32_R_TYPE (rel_backup.r_info) == R_NDS32_25_PCREL_RELA
	    ...
	    {
	      relocation = nds32_elf_ex9_reloc_insn (ptr, link_info);
	      insn =
		insn_with_reg | ((relocation >> shift) &
				 nds32_elf_irel_mask (&rel_backup));
	      bfd_putb32 (insn, contents + (ptr->order) * 4);
	    }
	  ...
	  else if (ELF32_R_TYPE (rel_backup.r_info) == R_NDS32_HI20_RELA)
	    {
	      ...
	      if (ptr->next && ptr->m_list && ptr->m_list == ptr->next->m_list)
		{
		  struct elf_link_hash_entry_mul_list *m_list;
		  ...
		  while (ptr->next && ptr->m_list && ptr->m_list == ptr->next->m_list)
		    {
		      /* Multiple sethi.  */
		      ptr = ptr->next;
		      size += 4;
		  ...
		  while (fix_ptr)
		      ...
		      while (temp_ptr)
			{
			  ...
			  insn = bfd_getb32 (contents + (temp_ptr->order) * 4);
			  if (insn == source_insn)
			    {
			      /* Fix the ex9 insn.  */
  ...
  if (ex9_export_file != NULL)
    nds32_elf_ex9_export (contents + 4, table_sec->size - 4);
  else if (update_ex9_table == 1)
    {
      ex9_export_file = ex9_import_file;
      rewind (ex9_export_file);
      nds32_elf_ex9_export (contents + 4, size);
    }
}
				...
				struct bfd_link_info *link_info)
{
  ...
  while (off < sec->size)
	...
	irel++;
      ...
      data_flag = nds32_elf_ex9_relocation_check (&irel, irelend, NULL,
						  sec, &off, contents);
      if (data_flag & DATA_EXIST)
	{
	  /* We save the move offset in the highest byte.  */
		  ...
		  || ELF32_R_TYPE (irel->r_info) == R_NDS32_LO12S2_RELA)
		{
		  ...
		  if (r_symndx < symtab_hdr->sh_info)
		  ...
		  else
		    {
		      ...
		      /* Rufus: Maybe there is a better way to get h and relocation */
		      RELOC_FOR_GLOBAL_SYMBOL (link_info, abfd, sec, irel,
					       r_symndx, symtab_hdr, sym_hashes,
					       h, sym_sec, relocation,
					       unresolved_reloc, warned);
		      insn =
			insn_with_reg | (((relocation + irel->r_addend) >> shift) &
					 nds32_elf_irel_mask (irel));
		    }
		}
	      else if ((ELF32_R_TYPE (irel->r_info) >= R_NDS32_SDA15S3
			&& ELF32_R_TYPE (irel->r_info) <= R_NDS32_SDA15S0)
		       || (ELF32_R_TYPE (irel->r_info) >= R_NDS32_SDA15S3_RELA
			   && ELF32_R_TYPE (irel->r_info) <= R_NDS32_SDA15S0_RELA)
		       || (ELF32_R_TYPE (irel->r_info) >= R_NDS32_SDA12S2_DP_RELA
			   ...
			   && ELF32_R_TYPE (irel->r_info) <= R_NDS32_SDA19S0_RELA))
		{
		  /* For instructions with relocation, we have to compare
		     their relocation type, instruction opcode, and target.  */
		  bfd_vma gp;
		  ...
		  if (r_symndx < symtab_hdr->sh_info)
		    {
		      ...
		      shndx = isym[r_symndx].st_shndx;
		      st_value = isym[r_symndx].st_value;
		      isec = elf_elfsections (abfd)[shndx]->bfd_section;
		      jrel = irel;
		      rel_backup = *irel;
		      relocation = isec->output_section->vma
				   + isec->output_offset + st_value - gp;
		      insn = insn_with_reg
			     | (((relocation + irel->r_addend) >> shift)
				& nds32_elf_irel_mask (irel));
		    }
		  else
		    {
		      /* Here is the external symbol, and compare their symbols.  */
		      bfd_boolean warned ATTRIBUTE_UNUSED;
		      bfd_boolean unresolved_reloc ATTRIBUTE_UNUSED;
		      asection *sym_sec;
		      jrel = irel;
...
nds32_elf_ex9_itb_base (struct bfd_link_info *link_info)
{
  ...
  bh = bfd_link_hash_lookup (link_info->hash, "_ITB_BASE_",
			     FALSE, FALSE, TRUE);
  return (_bfd_generic_link_add_one_symbol (link_info,
					    output_bfd,
					    "_ITB_BASE_",
